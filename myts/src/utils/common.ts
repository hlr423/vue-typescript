...
export const getDate = (fmt: any) => {
    ...
    for (const k in o) {if (new RegExp('(' + k + ')').test(fmt)) {
...
export const verifyPhone = (phone: string | number) => {
    ...
    const phones = phone.toString().trim();
    const toastStr = phones === '' ? '手机号不能为空~' : !reg.test(phones) && '请输入正确手机号~';
    return {
        errMsg: toastStr,
        done: !toastStr,
        value: phones
...
export const verifyStr = (str: string | number, text: string) => {
    const Strs = str.toString().trim();
    const toastStr = Strs.length ? false : `请填写${text}～`;
    return {
        errMsg: toastStr,
        done: !toastStr,
        value: Strs,
